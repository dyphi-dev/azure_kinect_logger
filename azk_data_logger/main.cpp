...
//char filename[100];
char execute_str[1000];
FILE* fp = NULL;
char ftp_portNo[100];
char ftp_target_addr[100];
char ftp_id[100];
char ftp_pw[100];
char ftp_dir[100];
int  device_id_balance;
int  device_id_gait;
int  device_id_s2s;
struct tm pLocal;
...
//
//    _finddatai64_t fd;
//    int64_t handle = _findfirst64(searching.c_str(), &fd); 
//
...
void ftp_uploader(void)
{
    //unsigned int prev_day = 0;
    
    for (;;)
    {
        //files = get_files_inDirectory(".\\", "*.mkv");
        while(files.size())
        {
            //std::cout << files[files.size()-1] << std::endl;
            std::string filename = files[files.size() - 1].c_str();
            files.pop_back();
            //ftp connection, upload
            
            std::cout << "make batch file for ftp upload : " << filename.c_str() << std::endl;
            fopen_s(&fp, "sftp_batch.bat", "w");
            fprintf(fp, "cd %s\n", ftp_dir);


            //if (prev_day != pLocal.tm_mday)
            //{
            //    //fprintf(fp, "mkdir %4d%02d%02d_%02d%02d\n", pLocal.tm_year + 1900, pLocal.tm_mon + 1, pLocal.tm_mday, pLocal.tm_hour, pLocal.tm_min);
            //    fprintf(fp, "mkdir %4d%02d%02d\n", pLocal.tm_year + 1900, pLocal.tm_mon + 1, pLocal.tm_mday);
            //}
            //prev_day = pLocal.tm_mday;
            //fprintf(fp, "cd %4d%02d%02d\n", pLocal.tm_year + 1900, pLocal.tm_mon + 1, pLocal.tm_mday);
            ...
            sprintf_s(execute_str, "psftp.exe -l %s -pw %s -P %s %s -b sftp_batch.bat", ftp_id, ftp_pw, ftp_portNo, ftp_target_addr);
            system(execute_str);
            std::cout << "upload completed" << std::endl;
            
            sprintf_s(execute_str, "del \"%s\"", filename.c_str());
            system(execute_str);
...
int main(int argc, char **argv)
{
    ...
    char filename_bal[100];
    char filename_gait[100];
    char filename_s2s[100];
    ...
    std::thread _t1(TCP_Msg_Handler); // thread UDP message handler
    std::thread _t2(ftp_uploader);
    recording_length = 60; //maximum recording length 60sec (1min)
    ...
    if (!fp) {
        perror("Configuration file opening failed");
        return EXIT_FAILURE;
    }

    fgets(ftp_target_addr, sizeof(ftp_target_addr), fp);
    fgets(ftp_portNo, sizeof(ftp_portNo), fp);
    fgets(ftp_id, sizeof(ftp_id), fp);
    fgets(ftp_pw, sizeof(ftp_pw), fp);
    fgets(ftp_dir, sizeof(ftp_dir), fp);
    fscanf_s(fp, "%d\n", &device_id_balance);
    fscanf_s(fp, "%d\n", &device_id_gait);
    fscanf_s(fp, "%d\n", &device_id_s2s);
    ...
    for (int i = 0; i < 100; i++)
    {
        if (ftp_target_addr[i] == 10)   ftp_target_addr[i] = 0;
        if (ftp_portNo[i] == 10)        ftp_portNo[i] = 0;
        if (ftp_id[i] == 10)            ftp_id[i] = 0;
        if (ftp_pw[i] == 10)            ftp_pw[i] = 0;
        if (ftp_dir[i] == 10)           ftp_dir[i] = 0;
    }

    std::cout << "ftp target address:" << ftp_target_addr << std::endl;
    std::cout << "ftp port No. : " << ftp_portNo << std::endl;
    std::cout << "ftp ID : " << ftp_id << std::endl;
    std::cout << "ftp password : " << ftp_pw << std::endl;
    std::cout << "ftp dir : " << ftp_dir << std::endl;
    std::cout << "device id (balance) : " << device_id_balance << std::endl;
    std::cout << "device id (gait) : " << device_id_gait << std::endl;
    std::cout << "device id (s2s) : " << device_id_s2s << std::endl;
    ...
    for (;;)
    {
        ...
        if ((recording_start_bal_flag) && (device_id_balance != -1))
        {
            time_t curTime = time(NULL); //recording current time
            localtime_s(&pLocal, &curTime);
            sprintf_s(filename_bal, "%4d%02d%02d_%02d_%02d_%02d_bal_%d.mkv",pLocal.tm_year + 1900, pLocal.tm_mon + 1, pLocal.tm_mday, pLocal.tm_hour, pLocal.tm_min, pLocal.tm_sec, device_id_balance);

            do_recording((uint8_t)device_id_balance, filename_bal, recording_length, &device_config, recording_imu_enabled, absoluteExposureValue, gain);
            files.push_back(filename_bal);
            exiting = false;
            recording_start_bal_flag = false;
        }

        if ((recording_start_gait_flag) && (device_id_gait != -1))
        {
            time_t curTime = time(NULL); //recording current time
            localtime_s(&pLocal, &curTime);
            sprintf_s(filename_gait, "%4d%02d%02d_%02d_%02d_%02d_gait_%d.mkv", pLocal.tm_year + 1900, pLocal.tm_mon + 1, pLocal.tm_mday, pLocal.tm_hour, pLocal.tm_min, pLocal.tm_sec, device_id_gait);

            do_recording((uint8_t)device_id_gait, filename_gait, recording_length, &device_config, recording_imu_enabled, absoluteExposureValue, gain);
            files.push_back(filename_gait);
            exiting = false;
            recording_start_gait_flag = false;
        }

        if ((recording_start_s2s_flag) && (device_id_s2s != -1))
        {
            time_t curTime = time(NULL); //recording current time
            localtime_s(&pLocal, &curTime);
            sprintf_s(filename_s2s, "%4d%02d%02d_%02d_%02d_%02d_s2s_%d.mkv", pLocal.tm_year + 1900, pLocal.tm_mon + 1, pLocal.tm_mday, pLocal.tm_hour, pLocal.tm_min, pLocal.tm_sec, device_id_s2s);

            do_recording((uint8_t)device_id_s2s, filename_s2s, recording_length, &device_config, recording_imu_enabled, absoluteExposureValue, gain);
            files.push_back(filename_s2s);
            exiting = false;
